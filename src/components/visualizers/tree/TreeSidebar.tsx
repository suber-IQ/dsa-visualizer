...
import { setTreeOperation } from "../../../redux/slices/treeSlice";
import { RootState } from "../../../redux/store";

type TreeOperation = "None" | "Traversals" | "BST" | "AVL" | "LCA";

const treeOperations: TreeOperation[] = ["None", "Traversals", "BST", "AVL", "LCA"];

const TreeSidebar = () => {
  const { operationType } = useSelector((state: RootState) => state.tree);
  const dispatch = useDispatch();

  const [isOpen, setIsOpen] = useState(true);
  const [selectedOperation, setSelectedOperation] = useState<TreeOperation>(operationType ?? "None");

  const handleSelectOperation = (event: React.ChangeEvent<HTMLSelectElement>) => {
    const value = event.target.value as TreeOperation;
    setSelectedOperation(value);
    dispatch(setTreeOperation(value));
  };
